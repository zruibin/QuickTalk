...
@interface QTAccountRegisterController ()
...
@implementation QTAccountRegisterController
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self initViews];
...
- (void)sendVerifyCodeEvent
{
    ...
    if ([account isMobileNumber]) {
        self.registerType = QuickTalk_ACCOUNT_PHONE;
...
- (void)registerButtonAction
{
    ...
    if (password.length == 0) {
        [QTMessage showErrorNotification:@"请输入密码" viewController:self];
        return ;
    }
...
- (void)submitData
{
    ...
    [QTAccountInfo requestRegisterUser:account password:password type:self.registerType completionHandler:^(QTAccountInfo *accountInfo, NSError *error) {
        if (error) {
            [QTProgressHUD showHUDWithText:error.userInfo[ERROR_MESSAGE]];
        } else {
            [QTProgressHUD showHUDSuccess];
            [[QTUserInfo sharedInstance] login:accountInfo password:password type:self.registerType];
            [self backAction];
        }
    }];
}
